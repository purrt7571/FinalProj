...
class BaseWindow(tk.Toplevel):
    ...
    def __init__(self, master: tk.Tk, min_width: int = 1500, minheight: int = 900) -> None:

        super().__init__(master=master)
        self.title("VectorSim")
        self.minsize(min_width, minheight)
    ...
    def get_resultant(self) -> None:
        ...
        self.resultant_xvar.set(value=f"{self.resultant_vct[0]: .4f}")  # type: ignore
        self.resultant_yvar.set(value=f"{self.resultant_vct[1]: .4f}")  # type: ignore
        self.resultant_rvar.set(value=f"{np.hypot(*self.resultant_vct): .4f}")
        self.resultant_thetavar.set(value=f"{np.rad2deg(np.arctan2(self.resultant_vct[1], self.resultant_vct[0])): .4f}")  # type: ignore
