...
class BaseWindow(tk.Toplevel):
    ...
    def rescale_graph(self) -> None:
        ...
        if np.any(self.resultant_vct) == False:
            ...
            x = [i[0] for i in self.vector_dict.values()] + [self.sum_given_vct[0]] + [self.resultant_vct[0]]
            y = [i[1] for i in self.vector_dict.values()] + [self.sum_given_vct[1]] + [self.resultant_vct[1]]  
...
ttk.Button(root, text = "Case 1").grid(column = 0, row = 0, sticky = "nsew", padx = 10, pady = (10, 0))
ttk.Button(root, text = "Case 2", command = lambda: BaseWindow(root)).grid(column = 0, row = 1, sticky = "nsew", padx = 10)
